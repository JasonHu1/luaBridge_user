...
#define TY_MEDIA_CALLBACK_VAL_MAX_LEN   32
#define TY_PHONE_NUMBER_MAX_LEN         32
#define TY_MEDIA_MESSAGE_ID_MAX_LEN     64
#define TY_MEDIA_TASK_ID_MAX_LEN        13
...
OPERATE_RET tuya_speaker_upload_media_start(OUT MEDIA_UPLOAD_CTX *p_ctx, 
                                                      IN CONST TY_MEDIA_ENCODE_T voice_encode, 
                                                      IN CONST TY_MEDIA_TARGET_T target, 
                                                      IN CONST CHAR_T *p_session_id, 
                                                      IN CONST BYTE_T *p_buf, 
...
OPERATE_RET tuya_speaker_http_upload_media_start(OUT MEDIA_UPLOAD_CTX *p_ctx, 
                                                             IN CONST TY_MEDIA_ENCODE_T voice_encode, 
                                                             IN CONST TY_MEDIA_TARGET_T target, 
                                                             IN CONST CHAR_T *p_session_id,
                                                             IN CONST CHAR_T *p_param_data);
OPERATE_RET tuya_speaker_http_upload_media_send(IN MEDIA_UPLOAD_CTX ctx, IN CONST BYTE_T *p_buf, IN CONST UINT_T buf_len);
OPERATE_RET tuya_speaker_http_upload_media_stop(IN MEDIA_UPLOAD_CTX ctx, BOOL_T force_stop);
...
typedef enum {
    TTS_KEEP_SESSION = 1,
    TTS_NO_KEEP_SESSION = 2,
}TY_CLOUD_FOLLOW_ACTION_E;

/*云端处理语音状态码*/
typedef enum {
    TY_CLOUD_SENT_COMMAND,//云端正确下发指令，被控制设备正常返回状态
    TY_CLOUD_NOT_SENT_COMMAND,//云端识别了指令，但是没有下发指令
    TY_CLOUD_NOT_IDENTIFY,//未识别语音
    TY_CLOUD_DEVICE_NOT_RESPONSE,//云端下发了指令，被控制设备未返回状态
}TY_CLOUD_PROCESS_CODE;
...
typedef struct
{
    ...
    CHAR_T              *p_req_body;
    CHAR_T              message_id[TY_MEDIA_MESSAGE_ID_MAX_LEN + 1];
    TY_MEDIA_TASK_TYPE_T task_type;
    CHAR_T              task_id[TY_MEDIA_TASK_ID_MAX_LEN + 1]; //ms时间戳计算
    CHAR_T              callback_val[TY_MEDIA_CALLBACK_VAL_MAX_LEN + 1];
    CHAR_T              phone_number[TY_PHONE_NUMBER_MAX_LEN + 1];
}TY_CLOUD_MEDIA_S;
/* 云端音视频资源获取回调 */
...
typedef VOID (*TY_SPEAKER_REV_CUSTOM_CB)(IN CONST CHAR_T *type, IN CONST ty_cJSON *json);
typedef VOID (*TY_CLOUD_PROCESS_CODE_CB)(TY_CLOUD_PROCESS_CODE code);
...
typedef struct {
    ...
    TY_SPEAKER_REV_CUSTOM_CB rev_custom_cb;
    TY_CLOUD_PROCESS_CODE_CB process_code_cb; 
...
OPERATE_RET tuya_speaker_mqtt_report_common(IN CONST CHAR_T *p_data);
